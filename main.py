...
def main():
    ...
    # -------------------------------
        ...
        if use_existing:
            projects = get_projects()
            if not projects:
                console.print("[red]No projects found on Dependency-Track.[/red]")
                sys.exit(1)
            ...
            if latest_version:
                ...
                console.print(f"[yellow]No existing version found for {proj_name}. Using default version 1.0[/yellow]")
                new_version = "1.0"
        ...
        # ✅ Create project (or reuse existing UUID) and upload
        project = get_or_create_project(proj_name, new_version)
        if not project:
            console.print(f"[red]❌ Project creation failed or already exists. Upload aborted.[/red]")
            sys.exit(1)
